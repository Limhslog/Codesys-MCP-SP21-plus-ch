...
> **This is a Chinese-friendly downstream fork.** It is **not** the original `luke-harriman/Codesys-MCP` and it is **not** the `phobicdotno/Codesys-MCP-SP21-plus` it forks from — it's a separate npm package with extra POU-text UTF-8 hardening on top.
>
> - **Original upstream:** [luke-harriman/Codesys-MCP](https://github.com/luke-harriman/Codesys-MCP) — the original architecture and tool set.
> - **Direct upstream:** [phobicdotno/Codesys-MCP-SP21-plus](https://github.com/phobicdotno/Codesys-MCP-SP21-plus) — the SP21+/SP22 watcher rewrite + release pipeline this fork inherits.
> - **This fork:** [Limhslog/Codesys-MCP-SP21-plus-ch](https://github.com/Limhslog/Codesys-MCP-SP21-plus-ch)
> - **npm:** [`codesys-mcp-sp21-plus-ch`](https://www.npmjs.com/package/codesys-mcp-sp21-plus-ch) — separate package; **does not** clash with the upstream `codesys-mcp-sp21-plus` if both are installed (the binary is named `codesys-mcp-sp21-plus-ch`).
> - **Maintainer:** Limhslog (this `-ch` fork).
>
> **Why fork.** Upstream's watcher relies on `system.execute_on_primary_thread()` to marshal work from a background thread back to the CODESYS UI thread. That API was **removed in CODESYS V3.5 SP21+**, so on SP21 / SP22 every tool call returned the same `Marshal error: The functionality 'system.execute_on_primary_thread(...)' is no longer supported` and the server was effectively unusable on current CODESYS releases. Several other upstream tools were also broken by unrelated script-engine API drift. The direct upstream (`phobicdotno/Codesys-MCP-SP21-plus`) fixes all of that and adds a release pipeline on top.
>
> **Why this further fork.** The `-ch` suffix stands for "Chinese": even after the direct upstream's `base64(UTF-8)` POU transport landed, several read paths and corner cases weren't pinned with regression tests, leaving silent mojibake risks for projects with Chinese comments / `°C` / 中文字符. This fork adds the missing test coverage on both modes (persistent + headless), the support matrix in [Chinese / Unicode support](#chinese--unicode-support), and the `parseAllPouCodeOutput` / `parsePouCodeOutput` extraction that makes the contract testable.
...
```bash
npm install -g codesys-mcp-sp21-plus-ch
```
...
```bash
codesys-mcp-sp21-plus-ch --print-config           # one entry per detected install
codesys-mcp-sp21-plus-ch --print-config --sp 21   # only the SP21 entry, named "codesys"
```

Output looks like this on a machine with two installs:

```jsonc
// Auto-generated by `codesys-mcp-sp21-plus-ch --print-config` on 2026-04-27.
...

{
  "mcpServers": {
    "codesys-sp21-patch5": {
      "command": "codesys-mcp-sp21-plus-ch",
      "args": [
...
- **User-scoped** (applies to every Claude Code session): `%USERPROFILE%/.claude.json`, or use `claude mcp add codesys codesys-mcp-sp21-plus-ch -- --codesys-path ... --codesys-profile ...`.
...
`codesys-mcp-sp21-plus-ch` is a standard **stdio** MCP server, so any MCP-capable client can drive it — not just Claude Code. OpenAI's Codex CLI stores MCP servers in a TOML file rather than a JSON `.mcp.json`.
...
**Translate the `--print-config` output into a `[mcp_servers.<name>]` table.** `--print-config` emits Claude Code's JSON, but the `command` and `args` are identical — only the wrapper format differs. Run `codesys-mcp-sp21-plus-ch --detect` first to get the exact path + profile string, then:
...
This is a Node.js MCP server published to npm as **`codesys-mcp-sp21-plus-ch`** (note the `-ch` suffix — distinct from the upstream `codesys-mcp-sp21-plus` package). It is **this Chinese-friendly downstream fork** — not the original `luke-harriman/Codesys-MCP`, not the direct upstream `phobicdotno/Codesys-MCP-SP21-plus`, and not a Python package. There is no `pip install`; the `.py` files under `src/scripts/` are CODESYS IronPython templates bundled inside the npm package itself.
...
- downloads the published tarball from <https://www.npmjs.com/package/codesys-mcp-sp21-plus-ch>
- installs it globally (`-g`) so the `codesys-mcp-sp21-plus-ch` binary is on your PATH (typically `%APPDATA%\npm\` on Windows)
- pulls in its 5 dependencies (`@modelcontextprotocol/sdk`, `commander`, `ssh2`, `uuid`, `zod`) automatically

Verify the install:

```bash
codesys-mcp-sp21-plus-ch --version
codesys-mcp-sp21-plus-ch --detect       # lists installed CODESYS versions
```
...
```bash
npm install -g codesys-mcp-sp21-plus-ch@latest
```

### Install from source (development / unreleased changes)

If you want to track this fork's `main` branch directly, contribute fixes, or pin to a specific commit instead of the published version:

```bash
git clone https://github.com/Limhslog/Codesys-MCP-SP21-plus-ch.git
cd Codesys-MCP-SP21-plus-ch
npm install
npm run build
npm link
```

`npm link` registers `dist/bin.js` as the global `codesys-mcp-sp21-plus-ch` binary, so the same `.mcp.json` snippet works. Edits to `src/` take effect after `npm run build`; Python script edits hot-reload from `dist/scripts/` without a rebuild.
...
- Run `codesys-mcp-sp21-plus-ch --detect` once to print every CODESYS install the server can see, with its profile name; copy the values from there into `.mcp.json` rather than guessing.
...
```bash
codesys-mcp-sp21-plus-ch --print-config --for-project "C:\path\to\MyMachine.project"
```
...
```bash
codesys-mcp-sp21-plus-ch --ssh-version 192.168.1.83
codesys-mcp-sp21-plus-ch --ssh-version myplc.lan --ssh-user pi
```
...
`open_project` runs an offline pre-flight (`projectinspectiondata.auxiliary` ZIP+XML — no CODESYS) that compares the project's saved profile against this server's `--codesys-profile`. Exact match proceeds silently; same-SP-different-patch proceeds with a one-line warning (CODESYS will pop its patch-difference dialog); SP mismatch refuses without opening so the project isn't dragged through a downgrade/upgrade conversion. The refusal includes a routing hint: pick a different MCP server entry or generate one with `codesys-mcp-sp21-plus-ch --print-config --for-project "<projectFilePath>"`. If the inspection itself fails (file missing, malformed .project, non-standard profile name), pre-flight falls through silently and the existing CODESYS open path produces its native error.
...
```bash
codesys-mcp-sp21-plus-ch --detect
```
...
- Original upstream: [luke-harriman/Codesys-MCP](https://github.com/luke-harriman/Codesys-MCP) — original architecture, the persistent-watcher concept, and the bulk of the upstream tool set.
- Direct upstream: [phobicdotno/Codesys-MCP-SP21-plus](https://github.com/phobicdotno/Codesys-MCP-SP21-plus) — Karstein Kvistad. SP21+/SP22 watcher rewrite, upstream-tool fixes, version-anchor + release pipeline, source-mirror export, base64(UTF-8) POU transport.
- This fork: [Limhslog/Codesys-MCP-SP21-plus-ch](https://github.com/Limhslog/Codesys-MCP-SP21-plus-ch) — Limhslog. Per-surface Chinese / UTF-8 support matrix, byte-exact round-trip pinning tests for persistent and headless modes, `parseAllPouCodeOutput` / `parsePouCodeOutput` extraction, defensive `ensure_ascii=True` on the bulk-read path.
