...
export function printConfig(installs: CodesysInstall[], opts: PrintConfigOptions = {}): string {
  ...
  if (filtered.length === 0) {
    if (opts.sp !== undefined) {
      throw new Error(
        `No CODESYS V3.5 SP${opts.sp} installation detected. Run \`codesys-mcp-sp21-plus-ch --detect\` to see what's available.`
      );
    }
    throw new Error(
      `No CODESYS installations detected. Looked under "C:\\Program Files" and "C:\\Program Files (x86)" for "CODESYS X.Y.Z.W"-named directories with a CODESYS\\Common\\CODESYS.exe inside. Run \`codesys-mcp-sp21-plus-ch --detect\` for raw output.`
    );
  }
  ...
  lines.push(`// Auto-generated by \`codesys-mcp-sp21-plus-ch --print-config\` on ${today}.`);
  ...
  for (const { name, install, isLast } of namedEntries) {
    ...
    lines.push(`      "command": "codesys-mcp-sp21-plus-ch",`);
    lines.push(`      "args": [`);
